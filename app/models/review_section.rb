class ReviewSection < ApplicationRecord
  ...
  def self.get_todays_review_list(user_id)
    ...
    section_ids = self.where(user_id: user_id).where("next_review_date <= ?", Date.current).pluck(:section_id)
  ...
  def self.get_todays_review_section(user_id, course, stage)
    ...
    # 復習する全sectionを取得
    user_review_sections = self.where(user_id: user_id).where(section_id: course.get_section_ids).where("next_review_date <= ?", Date.current)
